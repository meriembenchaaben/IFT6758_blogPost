...
<figcaption style="text-align:center;">Figure 5.3: Features Importance regarding XGboost</figcaption>
</figure>
 <br />
...
The model tunned and run with aall features presented the best performance if we take into consideration only these scores. Besides these metrics, our decision were taken also based on the four figures as quantitative metrics:
...
Unlike playoffs, the models have a comparable performance for regular seasons  when evaluated on both the validation and test sets.
This was expected given that the models were trained just on regular seasons and that there are some particular caracteristics of playoffs that make it different from regular seasons. 